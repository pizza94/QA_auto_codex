...
TC 작성, 수정, 실행 전에는 `docs/test-plan.md`, `docs/qualitystream-menu-map.md`, `docs/test-cases/qa-test-cases.csv`, `docs/work-log.md`를 먼저 읽고 현재 기준을 맞춘다.

TC 완료 전에는 `docs/work-log.md`의 날짜와 `docs/test-plan.md`의 실행 이력까지 갱신됐는지 확인한다.
...
- 2026-07-22 Chromium 단일 worker 실행: 통과
- DQI 필수 라벨은 현재 화면의 `*` 포함 텍스트 기준으로 보정
- 대분류/소분류 우클릭 컨텍스트 메뉴는 DOM `contextmenu` 이벤트 기준으로 안정화
- 수정 결과는 `standardManage/dqiList` API 재조회로 확인
- BR상세정보/연관BR 리포트 안내 문구는 화면 알림 방식 차이로 별도 재확인 필요
- 테스트 데이터는 `standardManage/dqiList` API로 정리했고 `AUTO_DQI_` 잔여 0건 확인
...
- 2026-07-22 Chromium 단일 worker 실행: 통과
- 신규 CTQ와 업로드 CTQ 모두 `검증대상관리 > 검증대상관리`의 `반영여부=Y` 대상 테이블 기준 컬럼 3개를 사용
- 신규 CTQ는 `QA_CTQ_004_NEW_20260722235152`, 컬럼건수 3으로 유지
- 업로드 CTQ는 `QA_CTQ_004_UPLOAD_CHROMIUM_20260722235209`, 컬럼건수 3으로 유지
- 업로드 성공 건수는 컬럼 3개 기준 3건 확인
- 테스트 데이터는 빈값 없이 생성하며, 실행 전 기존 `QA_CTQ_004_` 데이터는 정리
- 매핑 리포트 다운로드 검증은 이번 데이터 등록 중심 실행에서 제외되어 별도 회귀 필요
## 4. 실행 이력
...
9. `2026-07-22` - TC-001~004 Chromium 단일 worker 전체 실행 5건 통과. TC-003 안정화 및 TC-004 신규/업로드 3개 컬럼 등록 기준 갱신.
